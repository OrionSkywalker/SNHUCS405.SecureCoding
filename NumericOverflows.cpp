...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
  ...
  {
      //Check if overflow occurs during iterative steps as a result of exceeding the max for the type
      if (std::numeric_limits<T>::max()-increment < result) {
          //throw overflow error if conditions warrant
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
  ...
  for (unsigned long int i = 0; i < steps; ++i)
  {    //check for underflow. if result is in range and min+decrement are larger, then decrementing further will underflow
...
void test_overflow()
{
  ...
  //added try catch block to catch errors relating to overflow
  try {
...
void test_underflow()
{
  ...
  //added try catch block to handle error and report 
  try {
      ...
      //used to troubleshoot supposed underflow not being reported
      
      std::cout <<"Type min: " << +std::numeric_limits<T>::min() << std::endl;
      if (+result > +std::numeric_limits<T>::min()) {
          std::cout << "\tAn underflow has not occurred. Logic error on the part of original exercise designer." << std::endl;
      }
  }
